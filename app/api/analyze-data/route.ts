...
// Function to format the analysis with proper HTML - completely rewritten to avoid regex issues
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (
        ...
        trimmedLine.toUpperCase() === "DATA SUMMARY ANALYSIS" ||
        trimmedLine === "Data Summary Analysis"
      ) {
        if (currentSection) {
          formattedHTML += "</div>"
        }
        ...
        formattedHTML += `<h2>${headingText}</h2>\n<div class="section">`
        currentSection = "main"
      }
      // Check if this is a subheading
      else if (
        trimmedLine.startsWith("## ") ||
        trimmedLine === "KEY OBSERVATIONS" ||
        trimmedLine === "Key Observations" ||
        trimmedLine === "PATTERNS & TRENDS" ||
        trimmedLine === "Patterns & Trends" ||
        trimmedLine === "STATISTICAL INSIGHTS" ||
        trimmedLine === "Statistical Insights" ||
        trimmedLine === "RECOMMENDATIONS" ||
        trimmedLine === "Recommendations"
      ) {
        const headingText = trimmedLine.startsWith("## ") ? trimmedLine.substring(3) : trimmedLine
        formattedHTML += `<h3>${headingText}</h3>\n`
      }
