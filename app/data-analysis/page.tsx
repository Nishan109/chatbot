  ...
  const handleDataUpload = async (uploadedData: any[]) => {
    ...
    try {
      if (!uploadedData || uploadedData.length === 0) {
        throw new Error("No data provided or empty dataset")
      }

      setData(uploadedData)
      setActiveTab("analysis")
    } catch (err) {
      setError("Failed to process the uploaded data: " + (err instanceof Error ? err.message : String(err)))
      console.error(err)
    } finally {
          ...
          <nav className="space-y-2">
            {["upload", "stats", "analysis", "visualize"].map((tab) => (
              <Button
                ...
                disabled={data.length === 0 && tab !== "upload"}
              >
